import { compareMicromouseStrategies, DEFAULT_MOUSE_PHYSICS, DEFAULT_MOUSE_REALISM, type MousePhysics, type MouseRealism, type MouseStrategyId } from '../maze/micromouse';
...
export type BatchRunResult=Readonly<{seed:number;strategy:MouseStrategyId;success:boolean;reason?:string;searchTimeMs:number;searchCells:number;returnTimeMs:number;speedTimeMs:number;totalTimeMs:number;speedCells:number;speedDistanceCells:number;speedDiagonalCuts:number;effectiveAccelerationMps2:number;turns:number;revisits:number;exploredPercent:number;speedRouteQuality:number;collisions:number}>;
export type MicromouseBatchResult=Readonly<{completed:number;total:number;baseSeed:number;maze:Readonly<{width:number;height:number;generator:string;topology:string;mask:string;format:string}>;robotProfile:string;motion:Readonly<Pick<MousePhysics,'maxSpeedMps'|'accelerationMps2'|'turn90Ms'>>;realism:MouseRealism;results:readonly BatchStrategyResult[];runs:readonly BatchRunResult[]}>;
...
export function runMicromouseBatch(params:MazeParams,total:BatchSeedCount,motion:Pick<MousePhysics,'maxSpeedMps'|'accelerationMps2'|'turn90Ms'>,onProgress?:(result:MicromouseBatchResult)=>void,realism:MouseRealism=DEFAULT_MOUSE_REALISM):MicromouseBatchResult{
