...
export type BatchRunResult=Readonly<{seed:number;strategy:MouseStrategyId;success:boolean;reason?:string;searchTimeMs:number;searchCells:number;returnTimeMs:number;speedTimeMs:number;totalTimeMs:number;speedCells:number;speedDistanceCells:number;speedDiagonalCuts:number;turns:number;revisits:number;exploredPercent:number;speedRouteQuality:number;collisions:number}>;
