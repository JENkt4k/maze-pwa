...
export default function MicromouseOverlay({graph,events,eventIndex,cell,margin,width,height,showWalls,showFlood,showRoute}:Props){
  const frame=useMemo(()=>{
    ...
    let node=graph.start,heading:Heading='n',phase:'search'|'return'|'speed'='search',collision=false;
    for(const event of visible){phase=event.phase;collision=event.type==='collision';if(event.type==='sense')discovered.add(event.node);if(event.type==='collision')heading=event.heading;if(event.type==='move'){node=event.node;heading=event.heading;if(route[route.length-1]!==event.from)route.push(event.from);route.push(event.node);}if(event.type==='phase'){node=event.node;heading=event.heading;if(route[route.length-1]!==event.node)route.push(event.node);}}
    const target=phase==='return'?graph.start:graph.goals,distances=floodDistances(graph,knowledge,target,phase!=='speed');
    return{knowledge,discovered,route,node,heading,phase,distances,collision};
  ...
  return <svg className="micromouse-overlay-svg" viewBox={`0 0 ${width*cell+margin*2} ${height*cell+margin*2}`} aria-label={`Micromouse ${frame.phase} visualization`}>
    ...
    {frame.collision&&<circle className="mouse-collision" cx={current.x} cy={current.y} r={cell*.4}/>}<g className="mouse-robot" transform={`translate(${current.x} ${current.y}) rotate(${angle[frame.heading]})`}><path d={`M ${cell*.34} 0 L ${-cell*.24} ${cell*.23} L ${-cell*.24} ${-cell*.23} Z`}/><circle cx={-cell*.1} cy={0} r={cell*.08}/></g>
  </svg>;
}
