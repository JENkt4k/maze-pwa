...
type Props={graph:MazeGraph;state:MazeGameState;cell:number;margin:number;width:number;height:number;breadcrumbs:boolean;move:(target:NodeId)=>void};
...
export default function GameplayOverlay({graph,state,cell,margin,width,height,breadcrumbs,move}:Props){
  ...
  const moveDirection=(dx:number,dy:number)=>{const target=current.neighbors.find(id=>{const point=graph.nodes.get(id)!.position;return point.x===current.position.x+dx&&point.y===current.position.y+dy;});if(target)move(target);};
  ...
  const center=(id:NodeId)=>{const p=graph.nodes.get(id)!.position;return{x:(margin+(p.x+.5)*cell)/totalWidth*100,y:(margin+(p.y+.5)*cell)/totalHeight*100};};
  ...
  return <div ref={ref} className="gameplay-overlay" role="application" aria-label="Maze gameplay area" tabIndex={0} onKeyDown={keyDown} onPointerDown={down} onPointerUp={up}>
    ...
    {current.neighbors.map(id=>{const p=center(id);return<button key={id} type="button" aria-label={`Move to column ${graph.nodes.get(id)!.position.x+1}, row ${graph.nodes.get(id)!.position.y+1}`} onClick={()=>move(id)} style={{left:`${p.x}%`,top:`${p.y}%`}}/>;})}
