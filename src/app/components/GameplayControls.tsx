...
type Props={active:boolean;state:MazeGameState;breadcrumbs:boolean;setBreadcrumbs:(value:boolean)=>void;start:()=>void;pause:()=>void;quit:()=>void;restart:()=>void;hint:()=>void;newMaze:()=>void;shareChallenge:()=>void;difficulty:number;personalBestMs?:number};
...
export default function GameplayControls(props:Props){
  ...
  return <div className="game-controls" role="group" aria-label="Gameplay controls">
    ...
    <div className="game-action-grid">
      ...
      <button type="button" className="btn btn-sm" onClick={props.quit} disabled={!props.active||!['playing','paused'].includes(props.state.status)}>Quit</button>
    </div>
    ...
    {props.state.status==='complete'&&<section className="game-results" aria-label="Maze results">
      ...
      <div className="hstack"><button type="button" className="btn btn-primary" onClick={props.restart}>Play again</button><button type="button" className="btn" onClick={props.shareChallenge}>Share challenge</button><button type="button" className="btn" onClick={props.newMaze}>New maze</button></div>
    </section>}
