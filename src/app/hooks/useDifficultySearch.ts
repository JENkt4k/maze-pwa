...
export function useDifficultySearch(params: MazeParams, apply: (params: MazeParams) => void) {
  ...
  const { width, height, seed, g, b, tau, generator, mask, customMask, startCell, goalCell } = params;
  useEffect(() => {
    setSearching(false);
    setError(null);
    return () => { workerRef.current?.terminate(); workerRef.current = null; };
  }, [width,height,seed,g,b,tau,generator,mask,customMask,startCell,goalCell]);
