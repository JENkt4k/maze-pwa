...
export type MazeGameState={key:string;current:NodeId;route:NodeId[];moves:number;revisits:number;hints?:number;elapsedMs:number;status:GameStatus};

function initial(key:string,start:NodeId,status:GameStatus='idle'):MazeGameState{return{key,current:start,route:[start],moves:0,revisits:0,hints:0,elapsedMs:0,status};}
...
function load(key:string,start:NodeId,valid:ReadonlyMap<NodeId,unknown>):MazeGameState{
  try{const value=JSON.parse(localStorage.getItem(GAME_STORAGE_KEY)??'null') as Partial<MazeGameState>|null;
    if(value&&isValidState(value,key,valid))
      return{key,current:value.current,route:value.route,moves:value.moves!,revisits:value.revisits!,hints:Number.isFinite(value.hints)?Math.max(0,Math.trunc(value.hints!)):0,elapsedMs:value.elapsedMs!,status:value.status==='playing'?'paused':value.status as GameStatus};
  }catch{}
...
export function useMazeGame(graph:MazeGraph,key:string){
  ...
  useEffect(()=>{startedAt.current=null;elapsedAtStart.current=0;setHintNode(null);setState(load(key,graph.start,graph.nodes));},[key]);
  ...
  const restart=()=>{startedAt.current=Date.now();elapsedAtStart.current=0;setHintNode(null);setState(initial(key,graph.start,'playing'));};
  ...
  const move=(target:NodeId)=>{setHintNode(null);setState(current=>{
  ...
  const hint=()=>setState(current=>{
    if(current.status!=='playing')return current;
    const queue=[current.current],parent=new Map<NodeId,NodeId|null>([[current.current,null]]);let goal:NodeId|undefined;
    while(queue.length&&!goal){const id=queue.shift()!;if(goals.has(id)){goal=id;break;}for(const next of graph.nodes.get(id)?.neighbors??[])if(!parent.has(next)){parent.set(next,id);queue.push(next);}}
    if(!goal||goal===current.current)return current;let next=goal;while(parent.get(next)!==current.current)next=parent.get(next)!;setHintNode(next);return{...current,hints:(current.hints??0)+1};
  });
  return{state,hintNode,start,pause,restart,restore,move,hint};
}
