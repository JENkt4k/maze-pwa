...
export function useMicromouseBatch(params:MazeParams,motion:MouseMotion,realism:MouseRealism){
  const workerRef=useRef<Worker|null>(null),startedRef=useRef(0),[count,setCount]=useState<BatchSeedCount>(10),[running,setRunning]=useState(false),[result,setResult]=useState<MicromouseBatchResult|null>(null),[error,setError]=useState<string|null>(null),[durationMs,setDurationMs]=useState<number|null>(null);
