...
import { structuralMetrics } from './structural';
...
export function analyzeDifficultyV2(graph:MazeGraph):Difficulty2Analysis{
  ...
  const structural=structuralMetrics(context);
  const raw={activeCells:context.ids.length,shortestPathLength:length,shortestPathTurns:turns,shortestPathTurnRate:length>1?turns/(length-1):0,solutionJunctionCount:path.filter(node=>context.degree[node]>=3).length,...branch,...entropy,...deception,...structural};
...
export { structuralMetrics } from './structural';
