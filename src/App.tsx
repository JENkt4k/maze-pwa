...
import { registerSW } from "virtual:pwa-register";   // simple & robust
    ...
    >
      <style>
        ...
        @media (max-width: 840px) {
          body > div:first-child { grid-template-columns: 1fr !important; }
        }
      ...
      >
        <h2 style={{ margin: 0, fontSize: 20, display: "flex", justifyContent: "space-between" }}>
          Maze Controls
          {canInstall && (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <button style={btn} onClick={newMaze}>
            New Maze
          </button>
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#586174" }}>
                      {sv.params.width}×{sv.params.height}, seed {sv.params.seed}, g {sv.params.g.toFixed(2)}, b{" "}
                      {sv.params.b.toFixed(2)}, τ {sv.params.tau.toFixed(2)}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button style={btnSm} onClick={() => handleLoad(sv.id)}>
                      Load
                    </button>
                    <button style={btnDangerSm} onClick={() => handleDelete(sv.id)}>
                      Delete
                    </button>
                  </div>
                </div>
      ...
      >
        ...
        >
          <div id="print-maze-only" dangerouslySetInnerHTML={{ __html: svg }} />
          <div
            style={{
              border: "1px solid #e6e9ef",
              borderRadius: 12,
      ...
      {(offlineReady || needRefresh) && (
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {needRefresh && (
              <button
                onClick={() => updateSWRef.current?.(true)} // skip waiting & reload
                style={{ padding: "6px 10px", borderRadius: 8, border: "none", background: "#34d399", color: "#111" }}
              >
                Update
              </button>
            )}
